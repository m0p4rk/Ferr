...
function fetchTouristDetail(contentId) {
	...
	fetch(url)
		...
		.then(data => {
			...
			eventInfo.firstImage2 = firstImage2;
...
function submitSchedule() {
	fetchUserId().then(userId => {
		...
		// 현재 사용자 (일정 생성자)를 그룹원 목록에 추가
		if (!addGroupMembers.some(member => member.userId === userId)) {
			addGroupMembers.push({ userId }); // 사용자 ID만 추가
		}
		...
		fetch('/saveSchedule', {
			...
			.then(result => {
				...
				window.location.href = '/dashboard-schedule'; // 성공 후 리디렉션
			})
			.catch(error => {
